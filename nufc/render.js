...
function playerBadges(p, squadKey) {
  ...
  // A player can genuinely be both at once — e.g. a U21 who was named on a
  // senior bench, then went out on loan afterwards — so this isn't an
  // either/or: both the cross-squad badge and the status badge show
  // together when both apply, rather than the status one being suppressed.
...
function buildNotPlayingSection(groups) {
  ...
  all.forEach(g => g.players.forEach(p => grid.appendChild(buildStatusCard(p, g.kind))));
